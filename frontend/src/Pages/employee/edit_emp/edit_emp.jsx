  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        setRole(emp.role);
        setJoinDate(emp.join_date?.split("T")[0]);
        setStatus(emp.status);
      } catch {
        setMsg("Failed to load employee data");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400 animate-pulse font-medium">Loading profile data...</p>
        </div>
      </div>
    );
  }
        ...
        <motion.div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-6">
              ...
              <div>
                ...
                <p className="text-emerald-500/60 text-sm mt-1 font-medium tracking-wide">
                  Modify the professional credentials for <span className="text-white">{employeeCode}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <AnimatePresence>
            {msg && (
              ...
              >
                <div className={`text-sm ${msg.includes("failed") ? "text-red-400 bg-red-500/5 border-red-500/20" : "text-emerald-400 bg-emerald-500/5 border-emerald-500/20"} border px-4 py-3 rounded-xl flex items-center gap-3`} >
                  <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${msg.includes("failed") ? "bg-red-500" : "bg-emerald-500"}`} />
                  {msg}
                </div>
              </motion.div>
            )}
          ...
          <form onSubmit={handleSubmit} className="p-8 sm:p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
              ...
              {/* Join Date (Image Style Calendar) */}
              <div className="space-y-2">
              ...
              {/* Status Toggle (Image Style) */}
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-emerald-500/50 uppercase tracking-[0.2em] ml-1">Account Status</label>
